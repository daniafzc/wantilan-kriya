...
const filters = ["Semua", "Teknik & Bahan", "Tradisi & Makna", "Pasar & Bisnis", "Kolaborasi"];
...
export default function ArtikelPage() {
  ...
  const filteredArticles = ARTICLES.filter((article) => {
    const matchesFilter = activeFilter === "Semua" || article.category === activeFilter;
    const matchesSearch =
      searchQuery === "" ||
      ...
      <section className="bg-gradient-to-b from-[#f7e8d8] to-paper pt-12 md:pt-[60px] pb-10 md:pb-[50px] border-b border-line-soft">
        <div className="max-w-[1280px] mx-auto px-5 md:px-10">
          <div className="text-[13px] text-ink-muted mb-3.5">
            <Link href="/" className="text-terracotta hover:underline">Beranda</Link> · Artikel
          </div>
          <h1 className="font-serif text-3xl md:text-[48px] leading-tight mb-3">Semua Artikel</h1>
          <p className="text-base md:text-[17px] text-ink-soft max-w-[720px]">
            Cari pengetahuan praktis dari ratusan pengrajin Bali. Telusuri berdasarkan kata kunci atau saring per kategori.
          </p>
        </div>
      </section>

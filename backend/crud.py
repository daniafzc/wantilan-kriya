...
def _hitung_menit_baca(content: Optional[str], fallback: int = 3) -> int:
    """Hitung estimasi menit baca dari jumlah kata (200 kata/menit)."""
...
def create_artikel(db: Session, data: schemas.ArtikelCreate) -> models.Artikel:
    # Kalau menit_baca tidak diisi (default 3) dan ada content,
    # hitung otomatis dari panjang konten
    menit_baca = data.menit_baca
    if menit_baca == 3 and data.content:
        menit_baca = _hitung_menit_baca(data.content)
...
def update_artikel(
    ...
    # Kalau content diupdate tapi menit_baca tidak diisi ulang,
    # hitung ulang otomatis
    if "content" in update_data and "menit_baca" not in update_data:
        update_data["menit_baca"] = _hitung_menit_baca(update_data["content"])
